...
class PersonalInfo(models.Model):
    ...
    suffix = models.CharField(max_length=255, help_text="e.g. PhD", blank=True, null=True)
    locality = models.CharField(max_length=255, help_text="e.g. city such as Boston", blank=True, null=True)
    region = models.CharField(max_length=255, help_text="e.g. MA or Italy", blank=True, null=True)
    title = models.CharField(max_length=255, help_text="e.g. Developer", blank=True, null=True)
    email = models.EmailField(blank=True, null=True)
    linkedin = models.URLField(blank=True, null=True)
    facebook = models.URLField(blank=True, null=True)
    github = models.URLField(blank=True, null=True)
    site = models.URLField(blank=True, null=True)
    twittername = models.CharField(max_length=100, blank=True, null=True)
    image = models.ImageField(upload_to='static/resume/img/',blank=True, null=True)
    cv_pdf = models.FileField(upload_to='static/resume/img/', blank=True, null=True, help_text='Downloadable resume file')

    language = models.ManyToManyField(Language, through='MesLanguages', related_name="mes_language",
                                       through_fields=('personal', 'language'))
    programming_language = models.ManyToManyField('ProgrammingLanguage', through='MesLanguageProgrammation', related_name="mes_programming_language",
                                       through_fields=('personal', 'programming_language'))
...
class Skill(models.Model):
    ...
    text =  models.TextField()
    order = models.IntegerField(default=1)
    skillurl = models.URLField('Skill URL', null=True, blank=True)
    skillset = models.ForeignKey('Skillset',on_delete=models.CASCADE, related_name="mes_skills")
...
class MesLanguageProgrammation(models.Model):
    ...
    level = models.IntegerField(help_text='Choice between 1 and 5', choices=NIH_proficiency_scale, default=1)
...
class MesLanguages(models.Model):
    ...
    def __unicode__(self):
        return ' - '.join([self.language.name, str(self.level)])
